...
export default ({ options, label, selectChangeCallback, className, testId = '' }) => {
  ...
  return (
    <select-element class={className}>
      ...
      <select data-testid={testId} value={defaultOption ? defaultOption.value : options[0].value} className='selector' id={`select-${label}`} name={`select-${label}`} onChange={(event) => onChange(event, selectChangeCallback)}>
